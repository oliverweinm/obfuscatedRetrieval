...
from ..funcs.constants import CBORTABLE, EVENTTABLE, KOTLINTABLE, MASTERTABLE, OBFUSCATEDRETRIEVALTABLE
...
class SqLiteDatabase:
    ...
    def get_all_events_since(self, application, timestamp, chat_id):
        with self.session_scope() as session:
            subqry = session.query(cbor_event).filter(chat_event.timestamp > timestamp,
    ...
    """"Following comes the functionality used for the event Database regarding the obfuscatedRetrieval table:"""

    def create_obfuscatedRetrieval_table(self):
        metadata = MetaData()
        obfuscatedRetrieval_table = Table(OBFUSCATEDRETRIEVALTABLE, metadata,
                                 Column('id', Integer, primary_key=True),
                                 Column('feed_id', String),
                                 Column('seq_no', Integer),
                                 Column('application', String),
                                 Column('timestamp', Integer),
                                 Column('data', String))
        mapper(obfuscatedRetrieval_event, obfuscatedRetrieval_table)
        try:
            metadata.create_all(self.__db_engine)
        except Exception as e:
            logger.error(e)
    ...
    def insert_obfuscatedRetrieval_event(self, feed_id, seq_no, application, timestamp, data, public_key):
        with self.session_scope() as session:
            obj = obfuscatedRetrieval_event(feed_id, seq_no, application, timestamp, data, public_key)
            session.add(obj)
    ...
    def get_obfuscatedRetrieval_event_as_cbor(self, feed_id, seq_no):
        with self.session_scope() as session:
            subqry = session.query(cbor_event).filter(cbor_event.feed_id == feed_id,
                                                      cbor_event.seq_no == seq_no)
            res = subqry.first()
            row_feed_id, row_seq_no, event_as_cbor = cbor2.loads(res.event_as_cbor)
            if res is not None:
                return cbor2.loads(event_as_cbor)


    def get_obfuscatedRetrieval_event_data(self, feed_id, seq_no):
        with self.session_scope() as session:
            subqry = session.query(obfuscatedRetrieval_event).filter(obfuscatedRetrieval_event.feed_id == feed_id,
                                                                     obfuscatedRetrieval_event.seq_no == seq_no)
            l = []
            for row in subqry:
                if row.application == 'obfuscatedRetrieval':
                    l.append(row.data)
            return l
    ...
    def get_last_obfuscatedRetrieval_event_from_feed(self, feed_id):
        with self.session_scope() as session:
            subqry = session.query(obfuscatedRetrieval_event).filter(obfuscatedRetrieval_event.feed_id == feed_id)
            subqry_ordered = subqry.order_by(obfuscatedRetrieval_event.id.desc()).first()
            return(subqry_ordered)

    def get_all_obfuscatedRetrieval_event_feed_ids(self):
        with self.session_scope() as session:
            subqry  = session.query(obfuscatedRetrieval_event).filter()
            l = []
            for row in subqry:
                if row.application == 'obfuscatedRetrieval':
                    l.append(row.feed_id)
            return l
...
class obfuscatedRetrieval_event(object):
    def __init__(self, feed_id, seq_no, application, timestamp, data, public_key):
        self.feed_id = feed_id
        self.seq_no = seq_no
        self.application = application
        self.timestamp = timestamp
