...
class EventHandler(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self.__sqlAlchemyConnector.create_obfuscatedRetrieval_table()
    ...
    def add_event(self, event_as_cbor):
        try:
            ...
            #TODO:
            elif application =='obfuscatedRetrieval':
                if application_action == 'MASTER':
                    return
                elif application_action == 'request' or application_action == 'response':
                    data = content[1]['rpc']
                    public_key = content[1]['publicKey']
                    timestamp = content[1]['timestamp']
                    self.__sqlAlchemyConnector.insert_obfuscatedRetrieval_event(feed_id=feed_id, seq_no=seq_no, application=application,
                                                        timestamp=timestamp, data=data, public_key=public_key)
                elif application_action == 'prekey_bundle':
                    public_key = content[1]['publicKey']
                    timestamp = content[1]['timestamp']
                    prekey_bundle = content[1]['prekey_bundle']
                    self.__sqlAlchemyConnector.insert_obfuscatedRetrieval_event(feed_id, seq_no, application, timestamp,
                                                                                prekey_bundle, public_key)
                elif application_action == 'response_prekey_bundle':
                    public_key = content[1]['publicKey']
                    timestamp = content[1]['timestamp']
                    response_prekey_bundle = content[1]['response_prekey_bundle']
                    self.__sqlAlchemyConnector.insert_obfuscatedRetrieval_event(feed_id=feed_id, seq_no=seq_no, application=application,
                                                                                timestamp=timestamp, data=response_prekey_bundle, public_key=public_key)
                elif application_action == 'obfuscated_retrieval_request':
                    public_key = content[1]['publicKey']
                    timestamp = content[1]['timestamp']
                    cipher = content[1]['encrypted-pir-query']
                    self.__sqlAlchemyConnector.insert_obfuscatedRetrieval_event(feed_id=feed_id, seq_no=seq_no,
                                                                                application=application,
                                                                                timestamp=timestamp,
                                                                                data=cipher,
                                                                                public_key=public_key)
                elif application_action == 'confirmation-message':
                    public_key = content[1]['publicKey']
                    timestamp = content[1]['timestamp']
                    cipher = content[1]['confirmation-message']
                    self.__sqlAlchemyConnector.insert_obfuscatedRetrieval_event(feed_id=feed_id, seq_no=seq_no,
                                                                                application=application,
                                                                                timestamp=timestamp,
                                                                                data=cipher,
                                                                                public_key=public_key)
                elif application_action == 'obfuscated_retrieval_response':
                    public_key = content[1]['publicKey']
                    timestamp = content[1]['timestamp']
                    cipher = content[1]['encrypted-pir-response']
                    self.__sqlAlchemyConnector.insert_obfuscatedRetrieval_event(feed_id=feed_id, seq_no=seq_no,
                                                                                application=application,
                                                                                timestamp=timestamp,
                                                                                data=cipher,
    ...
    def get_obfuscatedRetrieval_event_data(self, feed_id, seq_no):
        return self.__sqlAlchemyConnector.get_obfuscatedRetrieval_event_data(feed_id, seq_no)

    def get_obfuscatedRetrieval_event_as_cbor(self, feed_id, seq_no):
        return self.__sqlAlchemyConnector.get_obfuscatedRetrieval_event_as_cbor(feed_id, seq_no)
    ...
    def get_all_obfuscatedRetrieval_event_feed(self):
        return self.__sqlAlchemyConnector.get_all_obfuscatedRetrieval_event_feed_ids()

    #TODO: Test functionality
    def get_last_obfuscatedRetrieval_event_from_feed(self, feed_id):
        return self.__sqlAlchemyConnector.get_last_obfuscatedRetrieval_event_from_feed(feed_id)
